...
unique_ptr<Algo_config> process_command_line(int ac, char* av[])
{
  ...
  try {
    ...
    desc.add_options()
      ...
      ("npc,n", po::value<int>(&config->npc)->required(), "set npc")
      ("algorithm,a", po::value<string>(&algo_string)->default_value("vfds"), "set algorithm variation, default to vfds. [vfds vfsi vri vci veu vec]")
      ...
      ("path,p", po::value<string>(&config->path)->required(), "set path")
